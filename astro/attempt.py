...
sigma_clip = SigmaClip(sigma=3, maxiters=10)
bkg_estimator = MedianBackground()
bkg_registered = Background2D(registered_image, (50, 50), filter_size = (3,3), sigma_clip = sigma_clip, bkg_estimator=bkg_estimator, exclude_percentile=5)
bkg_target = Background2D(targetlist, (50, 50), filter_size = (3,3), sigma_clip = sigma_clip, bkg_estimator=bkg_estimator, exclude_percentile=5)
...
registered_image_int = registered_image11.astype('uint8')
...
registered_image2 = registered_image11.astype('uint8')
...
#Let imageframe be the image to be subjected to object detection.
#the 1.5 value is the detection threshold of 1.5sigma, where sigma is the global background RMS

#The greatest isssue is to select the proper variable for the analysis of object detection. Given that
#in the library example, the background has alreayd been subtracted.
registered_imagesep = cv2.absdiff(registered_image11, bkg_registered.background)
image_data = registered_image1
blob_findermouseevent(image_data)


#This is an attempt to identify stars, will not be used right now.
                ...
                #im = morph.local_maxima(limg)
                #x1, y1 = np.where(im.T == True)
                #v = limg[(y1, x1)]
                #lim = 0.7
                #x2 = x1[v > lim]
                #y2 = y1[v > lim]

#Now we will attempt to include mouse functions for the selection of coordinates, the blob finder function should be now, not before.
#The parameters of the blob finder function have not been adjusted, so the identification and pattern recognition are not working properly yet.
