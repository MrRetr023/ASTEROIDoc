...
registered_imagesep = cv2.absdiff(registered_image1, bkg_registered.background)
image_data = registered_images1
histogram1()
plt.show()


bkg = sep.Background(registered_imagesep)

while True:
    try:
        objects = sep.extract(registered_imagesep, 1.5, err=bkg.globalrms)
        break
    except:
        sep.set_extract_pixstack(600000)
        objects = objects = sep.extract(registered_imagesep, 1.5, err=bkg.globalrms)
        print('DONE')
    break

print(len(objects))

image_data = registered_imagesep
histogram1()
...
#APPLY OBJECT DETECTION TO EACH INDIVIDUAL IMAGE
